...
function createThemeSelector() {
    const selection = UI.tag("select");
    for (const themeName of Theme.all()) {
        const option = UI.tag("option").sub(cap(themeName)).attr("value", themeName);
        if (themeName === Storage.theme) option.attr("selected", "");
        selection.add(option);
    }
    selection.on("input", () => {
        Theme.set(selection.dom.value);
        console.log(selection.dom.value);
    });
    return selection;
}
